...
class EventService:
    ...
    def create_event(self, contract:int, date_start:datetime, date_end:datetime, support_contact:int, location:str, attendee:int, note:str) -> object:
        event = Event(contract=contract, date_start=date_start, date_end=date_end, support_contact=support_contact, location=location, attendee=attendee, note=note)
        return self.repository.create_event(event)
    
    @require_permission("update_event")
    def update_event(self, contract_to_change:int, date_start_to_change:datetime, date_end_to_change:datetime, support_contact_to_change:int, location_to_change:str, attendee_to_change:int, note_to_change:str):
         return self.repository.update_event(contract_to_change, date_start_to_change, date_end_to_change, support_contact_to_change, location_to_change, attendee_to_change, note_to_change)
