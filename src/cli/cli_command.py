...
@click.option("--role", type=click.Choice(["management", "sales", "support"], case_sensitive=False), required=True)
...
def get_user_by_id(user_id):
    ...
    click.echo(f"ID number: {user.employee_number}\n"
               f"Name: {user.name}\n"
               f"Role: {user.role}")
...
@click.option("--status", type=click.Choice(["signed", "unsigned"], case_sensitive=False), required=False)
def create_contract(sale_contact, total_amount, amount_remaining_paid, customer_informations,status):
    contract = contract_controller.create_contract(sale_contact, total_amount, amount_remaining_paid, customer_informations, status)
...
@click.option("--status_to_change", type=click.Choice(["signed", "unsigned"], case_sensitive=False), required=False)
def update_contract(contract_id, sale_contact_to_change, total_amount_to_change, amount_remaining_paid_to_change, customer_informations_to_change,status_to_change):
    contract_controller.update_contract(contract_id, sale_contact_to_change, total_amount_to_change, amount_remaining_paid_to_change, customer_informations_to_change, status_to_change)
...
def get_contract_by_id(contract_id):
    ...
    click.echo(f"Contract N°:{contract.id__}\n"
...
def create_event(contract,date_start,date_end,support_contact,location,attendee,note):
    event_controller.create_event(contract,date_start,date_end,support_contact,location,attendee,note)
    click.echo("The event N° {event.id} id created.")

@click.command()
@click.option("--event_id",type=str,required=True)
@click.option("--contract_to_change",type=int,required=False)
@click.option("--date_start_to_change",type=click.DateTime(formats=["%Y-%m-%d"]),required=False)
@click.option("--date_end_to_change",type=click.DateTime(formats=["%Y-%m-%d"]),required=False)
...
def update_event(event_id,contract_to_change,date_start_to_change,date_end_to_change,support_contact_to_change,location_to_change,attendee_to_change,note_to_change):
    event_controller.update_event(event_id,contract_to_change,date_start_to_change,date_end_to_change,support_contact_to_change,location_to_change,attendee_to_change,note_to_change)
...
def filter_event(support_contact):
    ...
    for event in events_list:
        click.echo(f"Event N°{event.event_id},\n"
...
def assign_support_contact(event_id, support_contact):
    ...
    click.echo(f"User ID n°{support_contact} / Name: {event.support_contact.name} has been assigned to the event N°{event.id__}.")

@click.command()
@click.option("--id__",type=int,required=True)
def get_event_by_id(event_id):
    event = event_controller.get_event_by_id(event_id)
    click.echo(f"Event N°{event.id__},\n"
                   f"Contract N°{event.contract.contract.contract_id},\n"
                   f"Date Start: {event.date_start},\n"
                   f"Date End: {event.date_end},\n"
                   f"Support Contact: ID N°{event.support_contact.user.user_id},\n"
...
def create_customer(name,email,phone,company_name,last_update,sales_contact,information):
    customer = customer_controller.create_customer(name,email,phone,company_name,last_update,sales_contact,information)
    click.echo()
    click.echo(f"The customer ID N°{customer.id__} has been created.\n"
...
def update_customer(customer_id,name_to_change,email_to_change,phone_to_change,company_name_to_change,last_update,sales_contact_to_change,information_to_change):
    customer_controller.update_customer(customer_id,name_to_change,email_to_change,phone_to_change,company_name_to_change,last_update,sales_contact_to_change,information_to_change)
...
def get_customer_by_id(customer_id):
    ...
    click.echo(f"Customer ID n°{customer.id__},\n"
               f"Name : {customer.name},\n"
               f"Email : {customer.email},\n"
               f"Phone : {customer.phone}\n,"
